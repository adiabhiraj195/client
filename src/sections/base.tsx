import React from 'react'
import SectionHeading from '../components/section-heading';
import {BiHome, BiDownArrowAlt} from 'react-icons/bi';
import roundText from "../utilities/round-text.png";

const Base = () => {

  return (
    <div className='w-full'>
      <div className='relative'>
        <SectionHeading heading='INTRODUCTION' Icon= {BiHome}/>
      
        <h1 className='text-white text-7xl font-normal'>
          Say Hi from
          <br />
          <span className='text-[#28e98c]'>Aditya Raj</span>,
          <br />
          Webflow Developer
        </h1>
        
        <p className='text-gray-400 font-light text-xl '>I code beautifully simple things and i love what i do. Just simple like that!</p>

        <div className='flex justify-center items-center max-w-fit border rounded-full p-2 border-gray-500 cursor-pointer fixed right-1/4'>
          <BiDownArrowAlt className='absolute text-white font-bold text-2xl'/>
          <img src={roundText}></img>
        </div>
      </div>
    </div>
  )
}
export default Base;